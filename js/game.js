...
function checkMatch() {
  ...
  if (img1 === img2) {
    ...
    setTimeout(
      () => {
        card1.classList.remove("flipped"); //Flip cards back after 1.7s
...
function computerPlay() {
  ...
  //5 percent chance for computer to forget card from memory
  ...
  if (knownPair) {
    ...
    setTimeout(() => {
      ...
      const memoryMatch = computerMemory.find(
        (e) =>
          e.img === firstImg &&
          e.card !== first &&
...
function purgeMatchedFromMemory() {
  computerMemory = computerMemory.filter(
    (e) => !e.card.classList.contains("flipped"),
  );
}
...
function getRandomUnflippedCard(excludeCard) {
  const unflipped = [...cards].filter(
    (c) => !c.classList.contains("flipped") && c !== excludeCard,
  );
